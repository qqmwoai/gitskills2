...
//		$orderInfo = Util::getDataByCacheFirst('order',$id,array('Util','getSingelData'),array($select,'tg_order',$where));
	...
	*/  
	static function confirmOrder($orderinfo){
		...
		if($orderinfo['is_tuan']==1){
			$group = model_group::getSingleGroup($orderinfo['tuan_id'],"*");
			/*查询是否所有人都收货完成*/
			$goods = model_goods::getSingleGoods($orderinfo['g_id'], '*');
			if($goods['randrefundstatus']==1 && $goods['randrefundnum']>0){
				$tuan = pdo_fetchall("select id,status,price from".tablename('tg_order')."where tuan_id='{$orderinfo['tuan_id']}' and status=4");
